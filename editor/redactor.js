...
(function($) {
    ...
    // Functionality
    $.Redactor = Redactor;
    $.Redactor.VERSION = '10.2.6';
    $.Redactor.modules = ['alignment', 'autosave', 'block', 'buffer', 'build', 'button',
        'caret', 'clean', 'code', 'core', 'dropdown', 'file', 'focus',
        'image', 'indent', 'inline', 'insert', 'keydown', 'keyup',
        'lang', 'line', 'pagebreak', 'link', 'linkify', 'list', 'modal', 'observe', 'paragraphize',
    ...
    $.Redactor.opts = {
        ...
        autosaveName: false,
        autosaveIntervalIndex: false, // interval index
        ...
        buttons: ['formatting', 'bold', 'italic', 'underline', 'deleted', 'unorderedlist', 'orderedlist',
            'outdent', 'indent', 'image', 'file', 'link', 'horizontalrule', 'pagebreak', 'html'],
        ...
        shortcuts: {
            ...
            'ctrl+o, meta+o': { func: 'inline.format', params: ['deleted'] },
            'ctrl+i, meta+i': { func: 'inline.format', params: ['italic'] },
            //'ctrl+h, meta+h': { func: 'inline.format', params: ['superscript'] },
            'ctrl+s, meta+s': { func: 'autosave.load' },
            'ctrl+l, meta+l': { func: 'inline.format', params: ['code'] },
        ...
        fileTypes: ['application/atom+xml', 'application/octet-stream', 'application/pdf', 'application/xhtml+xml', 'application/zip', 'application/gzip', 'application/x-bittorrent', 'application/x-tex', 'audio/mp4', 'audio/aac', 'audio/mpeg', 'audio/ogg', 'audio/vnd.wave', 'image/svg+xml', 'text/plain', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'application/vnd.ms-powerpoint'],
        indentValue: 20,
        ...
        langs: {
            en: {
                ...
                underline: 'Underline',
                alignment: 'Alignment',
                filelink: 'Text of link (optional)',
    ...
    Redactor.fn = $.Redactor.prototype = {
        ...
        init: function(el, options)
        {
            ...
            var fullPath = $('script[src*=redactor\\.js]').attr('src');  // the js file path
            this.dir = fullPath.substring(0,fullPath.search('redactor.js'));   // the js folder path
        ...
        autosave: function()
        {
            return {
                ...
                enable: function()
                {
                    ...
                    console.log("this.autosave.interval = "+this.autosave.interval);
                    if(this.autosave.interval === false) {
                        console.log('Set this.autosave.interval = setInterval');
                        this.autosave.interval = setInterval(this.autosave.intervalEvent, this.opts.autosaveInterval * 1000);
                    }
                },
                intervalEvent: function()
                {
                    console.log('autosave.load() call: intervalEvent');
                    this.autosave.load();
                },
                onChange: function()
                {
                    if (!this.opts.autosaveOnChange) return;
                    console.log("autosaveOnChange = "+this.opts.autosaveOnChange);
                    this.autosave.load();
                },
                load: function()
                {
                    console.log('autosave... '+this.autosave.counter);
                    if (!this.opts.autosave) return;
                    if (this.autosave.process === true) return;
                    ...
                    this.autosave.counter++;

                    this.autosave.process=true;

                    var lastModify=$('form .formActions .lastModify');
                    lastModify.toggleClass('saving',true);
                    this.autosave.titleBuffer=document.title;
                    document.title = 'Автосохранение...';
                ...
                success: function(data)
                {
                    ...
                    document.title=this.autosave.titleBuffer;
                    console.log('Success...');

                    var lastModify=$('form .formActions .lastModify');
                    lastModify.toggleClass('saving',false);


                    if(lastModify.length && json.dateModify && json.timeModify) {
                        var timeSpan=lastModify.find('>span.time');
                        if(timeSpan.length) {
                            timeSpan.html('<span class="today">сегодня</span> в '+json.timeModify);
                        }
                    }
                    ...
                    // update this.autosave.html
                    this.autosave.html = this.autosave.source;

                    this.autosave.process=false;
                },
                disable: function()
                {
                    console.log('Clear this.autosave.interval...');
        ...
        build: function()
        {
            return {
                ...
                {
                    return (this.opts.name == false) ? this.$element.attr('id') : this.opts.name;
        ...
        button: function()
        {
            return {
                build: function(btnName, btnObject)
                {
                    ...
                    $button.append('<svg><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="' + this.dir + 'icons.svg#re-' + btnName + '"></use></svg>');
        ...
        caret: function()
        {
            return {
                ...
                set: function(contStart, offsetStart, contEnd, offsetEnd)
                {
                    // focus
                    // disabled in 10.0.7
                    //console.log(this.utils.browser('msie'));
                    if (!this.utils.browser('msie')) {
                        this.$editor.focus();
                    }

                    contStart = contStart[0] || contStart;
                    contEnd = contEnd[0] || contEnd;

                    if (this.utils.isBlockTag(contStart.tagName) && contStart.innerHTML === '')
                    {
                        contStart.innerHTML = this.opts.invisibleSpace;
                    }

                    if (contStart.tagName == 'BR' && this.opts.linebreaks === false)
                    {
                        var parent = $(this.opts.emptyHtml)[0];
                        $(contStart).replaceWith(parent);
                        contStart = parent;
                        contEnd = contStart;
                    }

                    // get current selection and Range object
                    this.selection.get();

                    try
                    {
                        // set selection borders
                        this.range.setStart(contStart, offsetStart);
                        this.range.setEnd(contEnd, offsetEnd);
                    }
                    catch (e) {
        ...
        file: function()
        {
            return {
                show: function()
                {
                    ...
                    this.upload.init('#redactor-modal-file-upload', this.opts.fileUpload, this.file.insert, 'file');
                ...
                insert: function(json, direct, e)
                {
                    ...
                    {
                        var text = $('#redactor-linktext').val();

                        if (typeof text == 'undefined' || text === '') text = json.filename;

                        var fileInfo = '';

                        if(typeof json.fileSize != 'undefined' || typeof json.fileType != 'undefined') {

                            fileInfo = ' <span class="fileInfo">(';

                            if(typeof json.fileType != 'undefined') {
                                fileInfo += json.fileType+' ';
                            }

                            if(typeof json.fileSize != 'undefined') {
                                fileInfo += json.fileSize+' Mb';
                            }
                        ...
                        link = '<a href="' + json.filelink + '" id="filelink-marker">' + text + fileInfo + '</a>';
                    }
        ...
        image: function()
        {
            return {
                show: function()
                {
                    ...
                    this.upload.init('#redactor-modal-image-droparea', this.opts.imageUpload, this.image.insert, 'image');
        ...
        inline: function()
        {
            return {
                ...
                format: function(tag, type, value)
                {
                    ...
                    // save editor selection and code state in buffer
                    this.buffer.set();
                    ...
                    // get current selection and Range object
                    // and save in properties: this.sel, this.range
                    this.selection.get();

                    // return true if range is closed
                    if (this.range.collapsed)
                    {
                ...
                formatMultiple: function(tag)
                {
                    ...
                    // add markers and save selection in this.savedSel
                    this.selection.save();
                ...
                {
                    // add selection markers and save selection in prop
                    this.selection.save();

                    //console.log(this.selection.toText());
        ...
        pagebreak: function()
        {
            return {
                ...
                {
                    var blocks = this.selection.getBlocks();
                    if (blocks[0] !== false && this.pagebreak.isExceptLastOrFirst(blocks))
                    {
                        if (!this.utils.browser('msie')) this.$editor.focus();
                        return;
                    }
                    ...
                    if (this.utils.browser('msie'))
                    {
                        this.pagebreak.insertInIe();
                    }
                    else
                    {
                        this.pagebreak.insertInOthersBrowsers();
                    }
                ...
                insertInIe: function()
                {
                    ...
                    this.insert.node(document.createElement('br').setAttribute('pagebreak',true));
                ...
                insertInOthersBrowsers: function()
                {
                    ...
                    var extra = '<p id="redactor-insert-pagebreak"><br /></p>';
                    if (this.opts.linebreaks) {
                        extra = '<br id="redactor-insert-pagebreak">';
                    }

                    document.execCommand('insertHtml', false, '<br pagebreak="true">' + extra);

                    this.pagebreak.setFocus();
                    this.code.sync();
                },
                setFocus: function()
                {
                    var node = this.$editor.find('#redactor-insert-pagebreak');
                    ...
                    if (target)
                    {
                        ...
                        {
                            this.$editor.focus();
                            this.pagebreak.setStart(target);
                        }
                    ...
                    {
                        node.removeAttr('id');
                        this.pagebreak.setStart(node[0]);
                    }
                },
                setStart: function(node)
                {
            ...
            return {
                ...
                loadTemplates: function()
                {
                    this.opts.modal = {
                        ...
                        + '<div class="row">'
                        + '<input type="text" id="redactor-linktext" placeholder="' + this.lang.get('filelink')+'" aria-label="' + this.lang.get('filelink') + '" />'
                        + '<input type="text" id="redactor-filename" placeholder="' + this.lang.get('filename') + '" arial-label="' + this.lang.get('filename') + '" />'
                        + '</div>'
        ...
        /**
         * Methods to working with selection
         */
        selection: function()
        {
            return {
                ...
                /*
                * Get selection and save in this.sel
                * Get Range object base on current selection and save in this.range
                * */
                get: function()
                {
                ...
                /*
                * Clear all ranges (clear all selections) of document and add a new Range from this.range
                * */
                addRange: function()
                {
                    try {
                        ...
                        console.log('Error removing all ranges.');
                    }
                ...
                /*
                * Add border markers to current selection
                * Save current selection in this.savedSel prop.
                * */
                save: function()
                {
                ...
                createMarkers: function()
                {
                    ...
                    this.selection.setMarker(this.range, marker1, true);

                    if (this.range.collapsed === false)
                    {
                        var marker2 = this.selection.getMarker(2);
                        this.selection.setMarker(this.range, marker2, false);

                        // Fix for Chrome58 Issues
                        if (this.utils.browser('chrome')) {
                            this.caret.set(marker1, 0, marker2, 0);
                        }
                        // End Chrome58 Issues
                    }
                ...
                setMarker: function(range, node, type)
                {
                    ...
                    try {
                        ...
                        // type - The caret location will be within this node
                        range.collapse(type);
                        // insert marker node
                        range.insertNode(node);
                ...
                /*
                * Search selection markers in $editor code
                * if markers is exists - set selection and remove markers
                * clear this.savedSel param
                * */
                restore: function()
                {
                    var marker1 = this.$editor.find('span#selection-marker-1');
                    var marker2 = this.$editor.find('span#selection-marker-2');
                    ...
                    if (marker1.length !== 0 && marker2.length !== 0)
                    {
                        this.caret.set(marker1, 0, marker2, 0);
                    }
                    else if (marker1.length !== 0)
                    {
                        this.caret.set(marker1, 0, marker1, 0);
                    }
                    else
                    {
                        this.$editor.focus();
        ...
        {
            return {
                init: function(id, url, callback, typeUpload)
                {
                    ...
                    if(typeUpload=='undefined') {
                        typeUpload='image';
                    }
                    this.upload.typeUpload = typeUpload;

                    this.upload.$placeholdler = $('<div id="redactor-droparea-placeholder" />').text(this.lang.get('upload_label'));

                    var acceptMimeTypes=false;

                    if(this.upload.typeUpload == 'image') {

                        this.upload.imageFields = {};

                        acceptMimeTypes=this.opts.imageTypes.join(',');

                    } else if(this.upload.typeUpload == 'file') {

                        this.upload.fileFields = {};

                        acceptMimeTypes=this.opts.fileTypes.join(',');
                        // specifiс fields
                        this.upload.$inputFileName = $('#redactor-filename');
                        this.upload.$inputLinkText = $('#redactor-linktext');

                    }

                    this.upload.$input = $('<input type="file" name="file" accept="'+acceptMimeTypes+'" />');
                ...
                traverseFile: function(file, e)
                {
                    ...
                    // disabled inputs
                    if(this.upload.typeUpload == 'file') {
                        this.upload.$inputLinkText.attr('disabled',true);
                        this.upload.$inputFileName.attr('disabled',true);
                    }

                    this.upload.$input.attr('disabled',true);
                ...
                setConfig: function(file)
                {
                    ...
                    {
                        this.upload.url = (this.upload.typeUpload == 'image') ? this.opts.imageUpload : this.opts.fileUpload;
                        this.upload.callback = (this.upload.typeUpload == 'image') ? this.image.insert : this.file.insert;
                    }
                },
                ...
                sendData: function(formData, e)
                {
                    ...
                    {
                        // append file name to send data
                        var fileName=false;
                        if(fileName = this.upload.$inputFileName.val()) {
                            this.upload.addFileFields('fileName', fileName);
                        } else {
                            console.log('no set filename');
                        }
